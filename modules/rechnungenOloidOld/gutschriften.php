...
<?
if($order=="kontakt"){
	$order="concat(kon.firma,kon.name)";
} else if($order=="text"){
	$order="gut.text";
...
if($term){
	$query=mysql_query("SELECT gut.id,gut.kontakt,gut.betrag,gut.text,gut.auszahlen FROM Rechnungen_gutschriften gut, Kontakte kon WHERE gut.bezahlt is NULL AND gut.kontakt = kon.id AND ".formatSearchString($term,array("kon.firma","kon.name","kon.vorname","gut.text","gut.betrag"))." ORDER BY $order");
} else {
...
echo mysql_error();
	if(@mysql_num_rows($query)>0)
	{
		...
		<tr>
			<td><b><a href=\"$PHP_SELF?order=kontakt\">Kontakt</a></b></td>
      <td><b><a href=\"$PHP_SELF?order=text\">Text</a></b></td>
      <td align=right><b><a href=\"$PHP_SELF?oder=betrag\">Betrag</a></b></td>
			<td><b><a href=\"$PHP_SELF?order=aktiv\">Aktiv</a></b></td>
		</tr>\n";
		for($i=0;list($id,$kontakt,$betrag,$grund,$aktiv)=mysql_fetch_row($query);$i++) {
			...
			if($aktiv==1)
				...
				$aktiv="Nein";
			print "<tr onmouseover=\"setPointer(this, 'over', '#$bgcolor', '#$_config_tbl_bghover', '')\" onmouseout=\"setPointer(this, 'out', '#$bgcolor', '#$_config_tbl_bghover', '')\" onclick=\"location.href='gutschriften_edit.php?id=$id&back=".urlencode($REQUEST_URI)."'\">
				<td width=250 valign=top bgcolor=\"#$bgcolor\"><a href=\"../kontakte/kontakt.php?id=$kontakt&back=".urlencode($REQUEST_URI)."\">".getKontakt($kontakt)."</a></td>

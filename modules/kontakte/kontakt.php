...
if(isset($id)) {
/*
	if(session_is_registered($kontakt)) {
		session_unregister($kontakt);
		unset($kontakt);
	}
  
  if(session_is_registered($kontaktpersonen)) {
    session_unregister($kontaktpersonen);
    unset($kontaktpersonen);
  }
 */ 
...
<body onLoad="javascript:error('<?=addslashes($error)?>'); document.getElementById('firma').focus();">
<? echo $debugLogString?>
<p class="titel">Kontakte:Kontakt</p>
<?
if(count($kontakt)>0) {
	print "<form method=post action=\"$PHP_SELF\" name=\"kontaktform\">
	<input type=\"hidden\" name=\"submit_form\" id=\"submit_form\" value=\"1\">
	<table border=0 cellpadding=0 cellspacing=0 width=\"95%\">";
	if($_config_kontakte_module1 || $_config_kontakte_module2 ){
		print "
			<tr>
				<td align=left valign=top >";
		if($_config_kontakte_module1) {
			foreach(split(";",$_config_kontakte_module1) as $module) {
    ...
    if($_config_kontakte_module2) {
      foreach(split(";",$_config_kontakte_module2) as $module) {
        include("modules/$module.inc.php");
      }
    }
		print "</td>
	...
	if($_config_kontakte_module3 || $_config_kontakte_module4 ){
		print "<tr>
				<td colspan=3 height=30>&nbsp;</td>
			</tr>
			<tr>
				<td align=left valign=top>";
    if($_config_kontakte_module3) {
      foreach(split(";",$_config_kontakte_module3) as $module) {
        include("modules/$module.inc.php");
      }
    }
		print "</td>
				<td width=\"*\">&nbsp;</td>
				<td align=right valign=top>";
    if($_config_kontakte_module4) {
      foreach(split(";",$_config_kontakte_module4) as $module) {
        include("modules/$module.inc.php");
      }
    }
		print "
	...
	if($_config_kontakte_module5 || $_config_kontakte_module6 ){
    print "<tr>
        <td colspan=3 height=30>&nbsp;</td>
      </tr>
      <tr>
        <td align=left valign=top>";
    if($_config_kontakte_module5) { 
			foreach(split(";",$_config_kontakte_module5) as $module) {
				include("modules/$module.inc.php");
			}
    }
    print "</td>
        <td width=\"*\">&nbsp;</td>
        <td align=right valign=top>";
    if($_config_kontakte_module6) {
			foreach(split(";",$_config_kontakte_module6) as $module) {
				include("modules/$module.inc.php");
			}
    }
    print "
	...
	if($_config_kontakte_module7 || $_config_kontakte_module8 ){
    print "<tr>
        <td colspan=3 height=30>&nbsp;</td>
      </tr>
      <tr>
        <td align=left valign=top>";
    if($_config_kontakte_module7) {
      foreach(split(";",$_config_kontakte_module7) as $module) {
        include("modules/$module.inc.php");
      }
    }
    print "</td>
        <td width=\"*\">&nbsp;</td>
        <td align=right valign=top>";
    if($_config_kontakte_module8) {
      foreach(split(";",$_config_kontakte_module8) as $module) {
        include("modules/$module.inc.php");
      }
    }
    print "
	...
	print "</table><br>
	<input type=\"submit\" name=\"submit_button\" value=\"Speichern\"> <input type=\"button\" onclick=\"javascript:location.href='".urldecode($back)."'\" value=\"Abbrechen\"> <input type=button onclick=\"javascript:location.href='delete.php?id=$id&back=".urlencode($back)."&backno=".urlencode($REQUEST_URI)."'\" value=\"Löschen\">\n"; 

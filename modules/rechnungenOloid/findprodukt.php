  ...
  <title><?php echo $_config_title?></title>
	...
	<!--
		function setValue(val){
			opener.document.getElementsByName('produkt[<?php echo $field ?>]')[0].value = val;
			self.close();
		}
	//-->
	</script>
...
if(@mysql_num_rows($query)>0)
{
  ...
  for($i=0;list($nr_int,$text1)=mysql_fetch_array($query);$i++) {
    ...
    print "<tr onmouseover=\"setPointer(this, 'over', '#$bgcolor', '#$_config_tbl_bghover', '')\" onmouseout=\"setPointer(this, 'out', '#$bgcolor', '#$_config_tbl_bghover', '')\" onclick=\"javascript:setValue('$nr_int');\">
			<td valign=top bgcolor=\"#$bgcolor\">$text1</td>";
  ...
  if($start>0){
    print "<a href=\"" . $_SERVER["PHP_SELF"] . "?field=$field&start=".($start-$_config_entrysperpage)."$attr\"><<<</a>";
  }
	if($term){

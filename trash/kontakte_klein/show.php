...
if($term){
	$query=mysql_query("SELECT $qryStr FROM Kontakte kon WHERE aktiv=1 AND ".formatSearchString($term,array("firma","name","vorname"))." ORDER BY concat(firma,name) LIMIT $start,$_config_entrysperpage");
	$attr="&term=$term";
} else {
	$query=mysql_query("SELECT $qryStr FROM Kontakte WHERE aktiv=1 ORDER BY concat(firma,name) LIMIT $start,$_config_entrysperpage");
}
if(@mysql_num_rows($query)>0)
{
  ...
  for($i=0;$result=mysql_fetch_array($query);$i++) {
    ...
    print "<tr onmouseover=\"setPointer(this, 'over', '#$bgcolor', '#$_config_tbl_bghover', '')\" onmouseout=\"setPointer(this, 'out', '#$bgcolor', '#$_config_tbl_bghover', '')\" onclick=\"location.href='detail.php?id=$id'\">\n";
		for($ii=0;$fields[$ii];$ii++) {
			if($fields[$ii]=="kontakt"){
				print "	<td valign=top bgcolor=\"#$bgcolor\"$style>".getKontakt($id)."</td>\n";
			} else if($fields[$ii]=="fullname"){
        print " <td valign=top bgcolor=\"#$bgcolor\"$style>".getKontaktName($id)." ".getKontaktVorname($id)."</td>\n";
        ...
        print " <td valign=top bgcolor=\"#$bgcolor\"$style>\n";
				...
				for($iii=0;list($domain_id,$domain)=mysql_fetch_row($query2);$iii++) {
					...
					echo "<a href=\"../../modules/domains/detail.php?id=$domain_id\">$domain</a>";
	      }
  	    print"</td>\n";
			} else {
  ...
  if($start>0){
	...
	if($term){
	  $query=mysql_query("SELECT count(*) FROM Kontakte WHERE aktiv=1 AND ".formatSearchString($term,array("firma","name","vorname")));
	} else {
